...
<? if(isset($msg)){ echo "<h2>$msg</h2>"; } ?>
  ...
  *</p>
  <p>
    ...
    <input name="keywords" type="text" id="keywords" size="35" maxlength="255" value="<?= stripslashes(reposted('keywords')); ?>"/>
  </p>
  <p>
...
// EndOAWidget_Instance_2204022
  ...
  <textarea id="doc" name="doc" rows="20" style="width:70%"><?= reposted($_POST['doc']); ?></textarea>
<? if(isset($error['doc'])){ echo "<span style=\"error\">{$error['doc']}</span>"; } ?>
  ...
  <p>
   ...
   <p>
    <label for="addphoto">Upload Photos:</label>
    <input type="checkbox" name="addphoto" id="addphoto">
  </p>
  <script type="text/javascript">
$('#addphoto').click(function () {
    $("#numofpics").toggle(this.checked);
});
  </script>

  <p id="numofpics"><label for="numofpix">Number of Images:</label><select name="numofpixs">
  <? 
  $i=1;
  while($i<=10){
	  echo "<option value=\"$i\"";
