...
/*if(!headers_sent()){
   session_start();
   if(!isset($_SESSION['admin'])){ $msg ="MUST BE LOGGED IN"; include_once('../apps/category/index.php'); exit; }
...
start_page('Category/List Page Controls');
$activity = "Category/List Page Controls"; 
?>
...
foreach($result as $row){ ?>
  ...
  <form action="<?=$_SERVER['../apps/category/REQUEST_URI']?>" method="post" name="update_form">
...
<form name="form2" method="post" action="<?=$_SERVER['../apps/category/REQUEST_URI'];?>">
<table align="center" class="list_category">
<tr>
