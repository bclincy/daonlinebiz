...
jQuery(document).delegate('.yk-toggler', 'click', function(){ 
	var toggler_name = jQuery(this).attr("name").replace("option_tree[","").replace("]","");
	var toggler_value = jQuery(".yk-toggler[name*="+toggler_name+"]:checked").val();
		
	// Turn off all .yk-toggle-able fields
	jQuery(".yk-toggle-able[class*="+toggler_name+"]").parents(".format-settings").slideUp();
		
	// Turn on fields that relate with toggler's value
	jQuery("."+toggler_name+"-"+toggler_value).parents(".format-settings").slideDown();
});
setTimeout(function() {jQuery(".yk-toggler:checked").click();}, 5);