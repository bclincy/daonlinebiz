...
mysql_select_db($database_dbcon, $dbcon);
$query_rsCustInv = sprintf("SELECT * FROM users JOIN states ON users.stateID=states.stateID JOIN invoice_meta inv ON custID=userID LEFT JOIN invoice_payments ON inv.ID=invoice_id WHERE inv.ID=%s", GetSQLValueString($cID_rsCustInv, "int"));
